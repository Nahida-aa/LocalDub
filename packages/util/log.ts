...
// 则在时间戳后追加 `[step] ` 前缀，即 `[ts] [step] line\n`。
 ...
 * 使用 enterWith，使本次设置对后续（同调用链的）await 串行执行的 step 可见，
...
/**
 * 进入某个 step 时调用：更新当前阶段名（保留 taskDir）。
 * 日志行随后会自动带 `[step] ` 前缀。pipeline-runner 在每个 step 开始前调用。
 */
export function setCurrentStep(step: string): void {
  const cur = als.getStore() ?? { taskDir: "" };
  als.enterWith({ ...cur, currentStep: step });
}
 ...
 * currentStep 非空时，在内容前加 `[step] ` 前缀。
 */
function appendToLog(taskDir: string | undefined, line: string, step?: string): void {
...
export function write_log(taskDir: string, line: string): void {
  const step = als.getStore()?.currentStep;
  appendToLog(taskDir, line, step);
}
