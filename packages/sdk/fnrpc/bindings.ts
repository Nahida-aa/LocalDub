...
export type EnqueueDirResult = {
	scanned: number,
	enqueued: number,
	skipped: number,
	errors: number,
	skipped_videos: string[],
};
...
export type StepName = "separate" | "separate_after" | "asr" | "asr_fix" | "sf_ocr_pre" | "sf_ocr" | "sf_ocr_fix" | "asr_ocr_pre" | "asr_ocr" | "asr_ocr_fix" | "translate" | "split_audio" | "tts" | "mix_audio" | "mix_video";
 ...
 */
export type Steps = Steps_Serialize | Steps_Deserialize;
 ...
 */
"import" | "enqueue_start" | "enqueue_continue" | "enqueue_import" | 
/**
 *  批量入队目录: 扫描本地目录顶层视频文件, 每个去重后入队一条 action=start 任务。
 *  (目录是入队动作的入参, 队列粒度仍是单视频 start 事件。)
 */
"enqueue_dir" | "list_queue" | "cancel_queue" | "status" | "get_group_list" | "get_workflow_ctx" | "generate_meta";
...
export type WorkflowArgs_Deserialize = {
	...
	continueFrom?: StepName | null,
	/**  目标步骤, pipeline 跑到此步骤后自动停止, 不指定则跑完所有步骤 */
	targetStep?: StepName | null,
...
export type WorkflowCtx_Deserialize = {
	...
	steps?: WorkflowStep_Deserialize[] | null,
	pipeline: string,
...
export type WorkflowCtx_Serialize = {
	...
	steps: WorkflowStep_Serialize[] | null,
	pipeline: string,
	last_run_pipeline: string | null,
	input: unknown,
	frame_rate: FrameRate,
...
export type WorkflowStep = WorkflowStep_Serialize | WorkflowStep_Deserialize;

export type WorkflowStep_Deserialize = {
	name: string,
	label: string,
	status?: StepStatus,
...
export type WorkflowStep_Serialize = {
	name: string,
	label: string,
	status: StepStatus,
	progress: number | null,
...
export type Procedures = {
  ...
  enqueue_dir: { kind: "mutate"; method: "POST"; input: Input; output: EnqueueDirResult; error: RpcErr };
...
export const __procedureMeta = {
  ...
  enqueue_dir: { kind: "mutate", method: "POST" },
