...
use subtitle_ocr::{FrameResult, MergeFramesArgs, MergeFramesResult};
use tracing::info;
...
fn main() -> Result<()> {
    ...
    let parsed: InputFrames = serde_json::from_str(&raw)
        .context("解析逐帧 JSON 失败（需为 FrameResult[] 或 {frames,meta}）")?;
    ...
    if let Some(out) = &cli.out {
        ...
        let json =
            serde_json::to_string_pretty(&result).context("序列化 MergeFramesResult 失败")?;
