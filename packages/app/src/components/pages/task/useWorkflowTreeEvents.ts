...
export interface WorkflowTreeHandlers {
...
/// path 相对化到 repo_root()（即 `workfolder/group/workflow/...`），与前端查询用的
...
/// 订阅某个 workflow 目录整棵文件树的变化事件，并分发给 handlers。
///
...
export function useWorkflowTreeEvents(videoDir: string, handlers: WorkflowTreeHandlers) {
  ...
  const cancel = consumeEventIterator<PathEvent>(fnrpc.watch_workflow_tree(videoDir), {
    ...
    onError: (err) => {
      warn("[useWorkflowTreeEvents] watch error:", err);
    },
  });
