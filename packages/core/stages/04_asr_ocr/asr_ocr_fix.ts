...
export async function stageAsrOcrFix(ctx: TaskCtx) {
  ...
  if (newTs.length > 0) {
    ...
    if (extracted > 0) {
      ...
      const yRange = ((ocrFramesData as any)._yRange ?? ctx.input?.stages?.asr_ocr?.yRange) as
        | [number, number]
        | undefined;
      ...
      const frameFiles = readdirSync(resampleDir)
        ...
        .sort();
      const ocrResults = await engine.ocrFrames(resampleDir, frameFiles, { textScore, yRange });
