//! import 阶段共享助手 (镜像 TS `packages/core/workflows/import/utils.ts`)。
//!
...
/// 判定结果: groupId / workflowId / source / yt-dlp 扩展参数 / 标题。
#[derive(Debug, Clone)]
pub struct AutoInfo {
    pub group_id: String,
    pub video_id: String,
...
/// 解析 groupId / workflowId / yt-dlp 扩展参数 (镜像 TS `autoGroupIdAndVideoId`)。
pub fn auto_group_id_and_video_id(url: &str) -> anyhow::Result<AutoInfo> {
    ...
    match source {
        ...
        VideoSource::Youtube | VideoSource::Bilibili => {
            ...
            if info.is_null() {
                ret.video_id = extract_video_id(url);
            } else {
...
/// 用 ffmpeg-next 探测视频帧率 (对齐 vision-lab subtitle-ocr 的 probe 用法)。
