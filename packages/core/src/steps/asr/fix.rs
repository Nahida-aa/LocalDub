//! asr_fix 阶段 (镜像 TS `packages/core/steps/asr/asr_fix.ts`)。
...
use crate::steps::utils::{
    asr_dir, ensure_dir, now_iso, resolve_language, set_step_anyhow, StepPatch, StepStatus,
};
...
pub fn step_asr_fix(ctx: &WorkflowCtx) -> anyhow::Result<()> {
    ...
    if !std::path::Path::new(&asr_file).exists() {
        return Err(anyhow!(
            "ASR file not found: {asr_file}; run ASR step first"
        ));
    }
...
fn segments_to_prompt(segments: &[crate::steps::asr::out::AsrSegment]) -> String {
