...
pub fn step_sf_ocr_fix(ctx: &WorkflowCtx) -> anyhow::Result<()> {
    ...
    if args.llm_fix.llm_fix {
        ...
        // (step 级 sf_ocr_fix.sourceLang 已移除, 统一到任务级)
        let legacy = ctx
            .input
            .get("steps")
            ...
            .is_some();
        if legacy {
            tracing::warn!(target: "sf_ocr", "steps.sfOcrFix.sourceLang 已移除, 请在 workflow.sourceLang 配置源语言");
        }
