...
/// 字幕对齐位置 (镜像 TS `packages/core/steps/mix_video/args.ts` alignmentList)
///
...
/// mix_video 阶段参数 (镜像 TS `packages/core/steps/mix_video/args.ts` MixVideoArgsSchema)
///
...
/// 以保证两种路径下默认值一致 (与 `input::steps::Asr` 同款处理)。
