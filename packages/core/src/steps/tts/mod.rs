//! tts 阶段 (镜像 TS `packages/core/steps/07_tts/tts.ts`)。
//!
...
//! 对长视频较慢。这是 Rust core 二进制编排模式下的已知取舍 (与其他 step 一致),
//! 后续可改为批量模式二进制消除。
...
use crate::steps::tts::args::{TtsArgs, TtsDevice, TtsRuntime};
use crate::steps::tts::out::{TtsFile, TtsSegment};
use crate::steps::utils::{
    ensure_dir, ffmpeg, find_release_bin, now_iso, probe_duration_ms, read_split_audio_timings,
    set_step_anyhow, tts_filepath, StepPatch, StepStatus,
};
...
/// 从 `ctx.input.steps.tts` 解析配置 (镜像 TS `ctx.input.steps.tts`)。
...
///
/// 仅用于非 cloud 运行时 (cloud 在 `step_tts` 中直接走 `VoxCPMCloud`, 不调用本函数)。
