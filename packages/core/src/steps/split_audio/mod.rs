...
//!
//! 镜像 TS `packages/core/steps/06_split_audio/split_audio.ts`。
...
use crate::steps::split_audio::out::{
    SplitAudioResult, SplitAudioSegment, SplitAudioTiming, SplitAudioTimingResult,
    TranslateResultMeta,
};
use crate::steps::split_audio::pad_segment::{pad_segments, SegmentBounds};
use crate::steps::utils::{
    ensure_dir, now_iso, probe_duration_ms, read_translation_result, resolve_language,
    set_step_anyhow, split_audio_path, split_audio_timings_path, subtitle_file_path,
    video_source_path, vocals_path, StepPatch, StepStatus,
};

pub use args::SplitAudioArgs;

/// 从 `ctx.input.steps.split_audio` 解析配置, 不存在时返回默认 (与 TS default 对齐)
pub fn read_args(ctx: &WorkflowCtx) -> SplitAudioArgs {
    ctx.input
        .get("steps")
...
mod tests {
    ...
    fn ctx_with_input(input: serde_json::Value) -> WorkflowCtx {
