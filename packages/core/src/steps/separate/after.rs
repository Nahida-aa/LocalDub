...
//!
//! 镜像 TS `packages/core/steps/separate/after.ts` (stepSeparateAfter), 纯 ffmpeg 编排。

use crate::context::WorkflowCtx;
use crate::steps::asr::args::AsrArgs;
use crate::steps::utils::{
    ffmpeg, now_iso, separate_after_dir, separate_dir, set_step_anyhow, StepPatch, StepStatus,
};

/// 读取 asr 阶段配置 (缺省用 AsrArgs::default, 对齐 TS `?? 默认值`)。
fn read_asr_cfg(ctx: &WorkflowCtx) -> AsrArgs {
    ctx.input
        .get("steps")
...
pub fn step_separate_after(ctx: &WorkflowCtx) -> anyhow::Result<()> {
    ...
    if use_separated && !matches!(mix_mode, crate::steps::asr::args::MixMode::Vocals) {
        ...
        match mix_mode {
            crate::steps::asr::args::MixMode::RawSum => {
            ...
            crate::steps::asr::args::MixMode::Sidechain => {
            ...
            crate::steps::asr::args::MixMode::Vocals => unreachable!(),
        }
    }
...
mod tests {
    ...
    fn ffmpeg_available() -> bool {
        std::process::Command::new(crate::steps::utils::ffmpeg_bin())
            .arg("-version")
            .output()
    ...
    fn separate_after_builds_bgm_and_sidechain() {
        ...
        let res = step_separate_after(&ctx);
