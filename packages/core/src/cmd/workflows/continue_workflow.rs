...
//!
//! 语义 (对齐 TS):
//! 1. 要求 `input.workflow.workflowDir` 指定已有任务目录, 否则报错;
//! 2. `setCtx` — 把当前 `input` 合并写回 `ctx.json` 的 `input` 字段
//!    (TS `setCtx(workflowDir, { input })`), 这样 `continueFrom` / `regenIndices` 等
...
pub fn continue_workflow(input: &Input) -> anyhow::Result<()> {
    let workflow_dir = input
        ...
        .ok_or_else(|| anyhow::anyhow!("continue 模式需要 input.workflow.workflowDir 指定已有任务目录"))?;

    if !std::path::Path::new(&workflow_dir).join("ctx.json").exists() {
        return Err(anyhow::anyhow!(
            "continue 模式找不到 {}/ctx.json, 确认 workflowDir 正确",
            workflow_dir
        ));
    }

    // setCtx: 把当前 input 合并进 ctx.json 的 input 字段 (镜像 TS setCtx(workflowDir, { input }))
    let mut ctx =
        read_ctx(&workflow_dir).map_err(|e| anyhow::anyhow!("读取 {}/ctx.json 失败: {e}", workflow_dir))?;
    ctx.input =
    ...
    let continue_from = input.workflow.as_ref().and_then(|t| t.continue_from.clone());
