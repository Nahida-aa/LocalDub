...
use crate::context::{read_ctx_from_value, write_ctx, Task, TaskCtx, TaskStage, VideoSource};
    ...
    match source {
        ...
        VideoSource::Youtube | VideoSource::Bilibili => {
            ...
            if download_subtitles {
                if let Err(e) = download_youtube_subtitles(url, &task_dir, yt_dlp_ext_args) {
...
fn find_task_video(task_dir: &std::path::Path, task_id: &str) -> Option<PathBuf> {
    ...
    std::fs::read_dir(task_dir).ok()?.find_map(|e| {
        let p = e.ok()?.path();
