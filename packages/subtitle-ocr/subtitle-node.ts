...
async function imgResizeToRgb(buf: Buffer, w: number, h: number): Promise<Buffer> {
...
export async function createSessions(detEp = "cpu"): Promise<OCRSessions> {
  ...
  const [det, cls, rec] = await Promise.all([
    ort.InferenceSession.create(join(modelDir, "ch_PP-OCRv3_det_infer.onnx"), {
      executionProviders: [detEp],
    }),
    ort.InferenceSession.create(join(modelDir, "ch_ppocr_mobile_v2.0_cls_infer.onnx"), {
      executionProviders: ["cpu"],
    }),
    ort.InferenceSession.create(join(modelDir, "ch_PP-OCRv3_rec_infer.onnx"), {
      executionProviders: ["cpu"],
    }),
  ]);
  return { det, cls, rec };
...
import { OcrBoxResult } from "./types";
export interface OCRNodeResult {
  text: string;
  segments: OcrBoxResult[];
...
export async function ocrFrameWithSessions(
  ...
  detFeed[sessions.det.inputNames[0]] = new ort.Tensor("float32", detInput, [
    1,
    3,
    resizedH,
    resizedW,
  ...
  const heatmapBytes = heatmapData.buffer.slice(
    heatmapData.byteOffset,
    heatmapData.byteOffset + heatmapData.byteLength,
  ) as ArrayBuffer;
  ...
  for (const b of boxes) {
    ...
    const crop = await imgCrop(imgBuf, {
      left: xMin,
      top: yMin,
      width: xMax - xMin,
      height: yMax - yMin,
    ...
    const { text, confidence } = ctcDecode(
      recTensor.data as Float32Array,
      recTensor.dims as number[],
    );
    ...
    const xys = absBox.map((p: number[]) => p[1]);
    ...
    segments.push({
      ...
      center: [
        xxs.reduce((a, b) => a + b, 0) / xxs.length,
        xys.reduce((a, b) => a + b, 0) / xys.length,
      ],
    });
  }
  ...
  return {
    text: totalText,
    segments,
    detInferenceMs: detMs,
    postprocessMs: ppMs,
    recInferenceMs: recMs,
    totalMs,
  };
}
...
if (require.main === module) {
  (async () => {
    ...
    const result = await ocrFrameWithSessions(imagePath, sessions, {
      textScore: 0.5,
      subtitleOnly: true,
    });
