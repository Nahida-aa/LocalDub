...
function runEvalOCR(
  ocrPath: string,
  label: string,
): { wer: number; cer: number; hyp_chars: number; ref_chars: number } {
...
async function runBenchmarkCommon(
  ...
  const ocrOutput: any = {
    audio_info: {
      duration:
        segments.length > 0 ? segments[segments.length - 1].end : Math.round(durationS * 1000),
    },
    result: {
      text: mergedText,
  ...
  if (hasPerFrameTimings) {
    console.log(
      `  det=${Math.round(totalDetMs)}ms post=${Math.round(totalPostMs)}ms rec=${Math.round(totalRecMs)}ms total=${Math.round(totalMs)}ms`,
    );
    console.log(
      `  avg/frame=${Math.round(totalMs / frameResults.length)}ms  inference=${inferenceS.toFixed(3)}s RTF=${rtf.toFixed(4)}`,
    );
  } else {
...
function ocrFramePython(
  ...
  return {
    lines: parsed.map((l: any) => ({ text: l.text, confidence: l.confidence })),
    inferenceMs: 0,
  };
}
...
function ocrFrameCpp(
  ...
  const r = spawnSync(bin, args, {
    ...
    env: {
      ...
      OCR_MODELS_DIR: OCR_MODELS_DIR,
      OCR_KEYS_PATH: OCR_KEYS_PATH,
    },
  });
  if (r.status !== 0) {
  ...
  const best = segs.reduce((a, b) => (a.confidence > b.confidence ? a : b), {
    text: "",
    confidence: 0,
  });
  return {
...
function extractFrames(
  ...
  outDir: string,
  fps: number = 1,
): { durationS: number; step: number; srcFps: number } {
  ...
  if (r.status !== 0) {
    throw new Error(
      `ffmpeg frame extraction failed (exit ${r.status}): ${r.stderr?.toString().slice(-300) || ""}`,
    );
  }
...
function runOCRBenchmarkPython(
  label: string,
  fps: number,
  textScore?: number,
  subtitleOnly?: boolean,
...
async function runOCRBenchmarkNode(
  label: string,
  fps: number,
  textScore?: number,
  subtitleOnly?: boolean,
  ...
  const result = runBenchmarkCommon(
    label,
    fps,
    "ocr-node",
    { textScore, subtitleOnly },
...
function runOCRBenchmarkCppOpencv(
  label: string,
  fps: number,
  textScore?: number,
  subtitleOnly?: boolean,
    ...
    ({ frameFiles, step, srcFps, frameDir }) => {
      ...
      return frameFiles.map((_, i) => {
        ...
        return {
          ...
          confidence: best.confidence || 0,
          totalMs:
            (data.detInferenceMs || 0) + (data.postprocessMs || 0) + (data.recInferenceMs || 0),
...
if (require.main === module) {
  const engine = process.argv.includes("--engine")
    ...
    : "python";
  const onlyLabel = process.argv.includes("--only")
    ? process.argv[process.argv.indexOf("--only") + 1]
    : null;
  const runs = process.argv.includes("--runs")
    ? parseInt(process.argv[process.argv.indexOf("--runs") + 1], 10)
    : 1;
  globalLabelSuffix = process.argv.includes("--label-suffix")
    ? process.argv[process.argv.indexOf("--label-suffix") + 1]
    : null;
  const labelOverride = process.argv.includes("--label-override")
    ? process.argv[process.argv.indexOf("--label-override") + 1]
    : null;
  ...
  globalOarModelSize = process.argv.includes("--oar-model-size")
    ...
    : null;
  console.log(
    `Engine: ${engine}  Only: ${onlyLabel ?? "all"}  Runs: ${runs}${globalLabelSuffix ? `  LabelSuffix: ${globalLabelSuffix}` : ""}${labelOverride ? `  LabelOverride: ${labelOverride}` : ""}${noNms ? `  NMS=OFF` : ""}${globalOarModelSize ? `  ModelSize: ${globalOarModelSize}` : ""}`,
  );
  ...
  async function main() {
    ...
    const runnerFor: Record<
      string,
      (label: string, fps: number, textScore?: number, subtitleOnly?: boolean) => any
    > = {
    ...
    console.log(
      "label           | fps | ts   | sz  | eng         | frames | segs | dur(s)  | inf(s)  | RTF    | WER%   | CER%   | hyp_ch | ref_ch",
    );
    console.log(
      "----------------|-----|------|-----|-------------|--------|------|---------|---------|--------|--------|--------|-------|-------",
    );
    for (const r of results) {
      ...
      console.log(
        `${l} | ${f} | ${ts} |  ${so} | ${eng} | ${fr} | ${s} | ${d} | ${inf} | ${rtf} | ${w}% | ${c}% | ${hc} | ${rc}`,
      );
    }
  }
