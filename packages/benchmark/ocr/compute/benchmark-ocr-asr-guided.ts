...
import {
  ...
  mergeFrames,
  dedupOverlap,
} from "@repo/core/stages/ocr/ocrMerge";
...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (args[i] === "--help") {
      console.log(
        "Usage: ASR_SOURCE=... ASR_PARAM=... bun run benchmark-ocr-asr-guided.ts [--text-score 0.45] [--subtitle-only] [--strategy mid1fps|end1fps|end2fps] [--label name]",
      );
      process.exit(0);
    }
  }
...
function ocrFrame(framePath: string): OCRLine[] {
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (lines.length === 0 && item.text) lines.push({ text: item.text, confidence: 1 });
    }
    return lines;
  } catch {
    return [];
...
function run(
  ...
  writeFileSync(
    join(resultDir, "metadata", "summary.json"),
    JSON.stringify(
      {
        label,
        desc,
        ocrSegments: ocrSegs.length,
        asrSegments: asrSegs.length,
        fixSegments: fixSegs.length,
        ocrHypChars: ocrText.length,
        asrHypChars: asrText.length,
        fixHypChars: fixText.length,
  ...
  writeFileSync(
    join(resultDir, "metadata", "ocr.json"),
    JSON.stringify(
      {
        ...
        result: {
          text: ocrText,
          segments: ocrSegs.map((s) => ({ text: s.text, start: s.start, end: s.end })),
        },
      },
      null,
      2,
  ...
  writeFileSync(
    join(resultDir, "metadata", "asr_ocr_fix.json"),
    JSON.stringify(
      {
        _engine: "asr_ocr",
        _strategy: STRATEGY,
...
if (require.main === module) {
  ...
  for (let i = 0; i < asrSegs.length; i++) {
    ...
    if (i === 0) {
      ...
      for (let t = Math.round(asr.start); t <= Math.round(asr.end); t += 100)
    ...
    for (const frameMs of frames) {
      const framePath = join(
        TMP_DIR,
        `${STRATEGY}_${i.toString().padStart(4, "0")}_${frameMs}.jpg`,
      );
      if (!extractFrame(VIDEO_PATH, frameMs, framePath)) continue;

      const lines = ocrFrame(framePath);
      ocrCount++;
      const best = lines.reduce((a, b) => (a.confidence > b.confidence ? a : b), {
        text: "",
        confidence: 0,
      });
      ...
      if (best.text && best.confidence > threshold) {
        asrResults.push({
          text: best.text,
          start: i === 0 ? Math.max(0, frameMs - 90) : asr.start,
          end: asr.end,
        });
  ...
  const ocrSegs_merged = mergeFrames(rawFrames, {}).segments.map((s) => ({
    text: s.text,
    start: s.start,
    end: s.end,
  }));
  ...
  run(
    ...
    asrSegs_merged,
    rawFrames,
    `ASR ${hop} (${ocrCount} OCR calls, ${elapsed.toFixed(1)}s)`,
    asrSegs,
    ocrCount,
    elapsed,
  );
