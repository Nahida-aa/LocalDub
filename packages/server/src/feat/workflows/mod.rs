...
#[fnrpc::rpc_query]
pub async fn get_workflow_ctx(video_dir: String) -> Result<WorkflowCtx, String> {
    let path = repo_root().join(&video_dir);
    context::read_ctx(
        &path
            .to_str()
            .ok_or_else(|| format!("Invalid video_dir: {}", video_dir))?,
    )
}

#[fnrpc::rpc_mutate]
pub async fn continue_workflow(video_dir: String, from_step: String) -> Result<(), String> {
    let abs_video_dir = repo_root().join(&video_dir);
    let abs_video_dir_str = abs_video_dir
        .to_str()
        .ok_or_else(|| "invalid video_dir".to_string())?
        .to_string();
    eprintln!(
        "[continue_workflow] base_dir={} video_dir={video_dir} abs={abs_video_dir_str}",
        repo_root().display()
    );

    // 读 ctx.json 的 input 字段作为续跑基准配置 (仅改写 workflow 相关字段, 其余原样保留)。
    let ctx_path = abs_video_dir.join("ctx.json");
    let ctx_raw =
    ...
    workflow["videoDir"] = serde_json::Value::String(abs_video_dir_str.clone());
...
/// 返回相对 `workfolder` 的 video_dir, 供前端跳转任务页。
#[fnrpc::rpc_mutate]
pub async fn start_workflow(url: String) -> Result<String, String> {
    ...
    // 只跑导入 (拷贝/下载 + 探测 + 写 ctx.json), 拿到 video_dir 立即返回;
    ...
    let abs_video_dir = match ctx {
        Ok(Ok(ctx)) => ctx.workflow.video_dir,
        Ok(Err(e)) => return Err(e),
        Err(e) => return Err(format!("import 任务崩溃: {e}")),
    };

    // 后台跑完整 pipeline (不阻塞 RPC); 失败仅记日志, 任务页可续跑。
    let video_dir = abs_video_dir.clone();
    tokio::task::spawn_blocking(move || {
        if let Err(e) = ld_core::workflows::pipeline::run_pipeline(&video_dir) {
            tracing::error!("start_workflow pipeline 失败 ({video_dir}): {e:#}");
        }
    });

    // 返回相对 workfolder 的 video_dir (供前端导航 /group/<group>/<workflow>)。
    std::path::Path::new(&abs_video_dir)
        .strip_prefix(repo_root())
        .map(|p| p.to_string_lossy().into_owned())
        .map_err(|_| format!("video_dir 不在 workfolder 内: {abs_video_dir}"))
}
